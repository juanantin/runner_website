...
src/
  ...
  lib/
    ...
    ui-store.tsx        # React context for the toast
```
...
- `src/assets/hero-city.jpg` — the cinematic rainy-city/hooded-protagonist
  plate. Used as the hero background (`Hero.tsx`) and as the trailer
  video's poster image (`TrailerSection.tsx`). A static `next/image`
  import, so Next.js generates optimized/responsive variants automatically.
